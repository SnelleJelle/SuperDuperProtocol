...
// use:                      169.254.255.254:8080

const int chipSelect = 4;

// pins
int devicePins[] = {0, 1, 2, 3, 4, 5}; //pin of device with id 5 => devicePins[5] = pinNr;

String deviceTypes[] = {"", "led", "led", "wheel", "wheel", "distance_sensor"}; //type of device with id 5 => deviceTypes[5] = deviceType;

boolean ledStates[] = {false, false, false}; // state of led with id 2 => ledStates[2] = true/false
...
void serve_list(EthernetClient client) {
  ...
  client.println("{ \"parts\":[ { \"id\":\"1\", \"extras\":[ { \"state\":0 } ], \"type\":\"led\" }, { \"id\":\"2\", \"extras\":[ { \"state\":0 } ], \"type\":\"led\" }, { \"id\":\"3\", \"extras\":[ { \"direction\":\"stop\" }, { \"position\":\"left\" } ], \"type\":\"wheel\" }, { \"id\":\"4\", \"extras\":[ { \"direction\":\"stop\" }, { \"position\":\"right\" } ], \"type\":\"wheel\" }, { \"id\":\"5\", \"extras\":[ { \"distance\":67 } ], \"type\":\"distance_sensor\" } ] }");
}
...
boolean isDistanceSensor(int sensor_id) {
  if (deviceTypes[sensor_id] == "distance_sensor") {
    return true;
  }
  return false;
}

// when posisble use serve_gui_seperated(client); instead
void serve_gui(EthernetClient client) {
  // TODO: serve files from filesystem (micro-sd card)
